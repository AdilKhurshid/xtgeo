...
def well_from_rms(
    ...
    activated (through the rms environment).
...
class Well:
    ...
    def get_cell_intersections(
        ...
        Note:
            Rows where any of X, Y, Z, or MD is NaN (common for undefined
            samples in well data) are automatically removed before
            processing.

        Raises:
            ValueError: If the well has no MD log, if ``sampling_step`` is
                not positive, or if ``refine_iters`` is negative.  A
                trajectory with fewer than 2 finite samples returns an empty
                DataFrame (not an error).
        ...
        # Drop rows where any coordinate or MD is NaN (undefined separator
        # rows are stored as NaN by convention, see class docstring).
        finite_mask = (
            np.isfinite(xv) & np.isfinite(yv) & np.isfinite(zv) & np.isfinite(mdv)
        )
        xv = xv[finite_mask]
        yv = yv[finite_mask]
        zv = zv[finite_mask]
